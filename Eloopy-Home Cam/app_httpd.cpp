...
// Declare the function defined in the .ino file
extern void lightsON_OFF();
extern void ServoLeft();
extern void ServoRight();
extern void reset_Servo_Light();
...
static esp_err_t lights(httpd_req_t *req) {
  lightsON_OFF();  // Call the Arduino function
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  return httpd_resp_send(req, "OK", strlen("OK"));
}

static esp_err_t servo_left(httpd_req_t *req) {
  ServoLeft();  // Call the Arduino function
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  return httpd_resp_send(req, "OK", strlen("OK"));
}

static esp_err_t servo_right(httpd_req_t *req) {
  ServoRight();  // Call the Arduino function
  httpd_resp_set_hdr(req, "Access-Control-Allow-Origin", "*");
  return httpd_resp_send(req, "OK", strlen("OK"));
}
...
void startCameraServer() {
  ...
  config.server_port = 7117;
  config.ctrl_port = 7117;
 ...
 httpd_uri_t servo_left_uri = {
  ...
  if (httpd_start(&camera_httpd, &config) == ESP_OK) {
    ...
    httpd_register_uri_handler(camera_httpd, &servo_left_uri);
    httpd_register_uri_handler(camera_httpd, &servo_right_uri);
  }


  config.server_port = 7118;
  config.ctrl_port = 7118;
